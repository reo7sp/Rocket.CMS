...
Poco::LRUCache<std::string, std::string>& CacheManager::getGeneralCache() {
	static LRUCache<string, string> cache(ConfigTools::getConfig().getUInt("cache.general.size"));
	return cache;
}

Poco::LRUCache<std::string, std::string>& CacheManager::getPrivateCache() {
	static LRUCache<string, string> cache(ConfigTools::getConfig().getUInt("cache.private.size"));
	return cache;
}
